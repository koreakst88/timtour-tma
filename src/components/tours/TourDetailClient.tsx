...
export function TourDetailClient({
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* ФОТО — полная ширина, без отступов сверху */}
          ...
          {previewReviews.length > 0 ? (
            <div className="space-y-3">
              ...
              {reviews.length > 3 && (
                <button
                  type="button"
                  className="w-full rounded-2xl border border-[#FF6B35] px-4 py-3
                    text-sm font-bold text-[#FF6B35]
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-5 py-4 z-20">
        <div className="flex items-center justify-between max-w-md mx-auto">
          ...
          <Link href={`/booking/${tour.id}`}>
            <button
              type="button"
              className="bg-[#FF6B35] text-white px-7 py-4 rounded-2xl
                font-bold text-base
                active:scale-95 transition-transform
